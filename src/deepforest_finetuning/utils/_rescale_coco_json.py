...
def rescale_coco_json(
    ...
    if source_image_path is None and source_image_shape is None:
        raise ValueError(
            "Either source_image_path or source_image_shape must not be None."
        )
    ...
    if source_image_path is not None:
        with rasterio.open(source_image_path) as image:
            transform = image.transform
            source_pixel_size = np.abs(
                np.array([transform[0], transform[4]], dtype=np.float64)
            )
    else:
        source_image_shape = cast(npt.NDArray, source_image_shape)
        source_pixel_size = (
            target_image_shape / source_image_shape
        ) * target_pixel_size
    ...
    for annotation in coco_json["annotations"]:
        ...
        bounding_box[[0, 2]] = (
            bounding_box[[0, 2]] * source_pixel_size[0] / target_pixel_size[0]
        )
        bounding_box[[1, 3]] = (
            bounding_box[[1, 3]] * source_pixel_size[1] / target_pixel_size[1]
        )
