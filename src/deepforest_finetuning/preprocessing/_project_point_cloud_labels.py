...
def project_point_cloud_labels(  # pylint: disable=too-many-locals, too-many-statements
    ...
    with rasterio.open(config.image_path) as image:
    ...
    point_cloud = read(config.point_cloud_path)
    ...
    label_image = modal(label_image, footprint=np.ones((3, 3), dtype=np.int32))
    ...
    if config.label_image_output_path is not None:
        Path(config.label_image_output_path).parent.mkdir(exist_ok=True, parents=True)

        with rasterio.open(
            config.label_image_output_path,
            "w",
            driver="GTiff",
    ...
    if len(image_path.stem) >= 8 and (image_path.stem[:8]).isnumeric():
        date_prefix = Path(config.image_path).stem[:8]
        ...
        "images": [
            {
                ...
                "width": image_width,
                "height": image_height,
                "file_name": Path(config.image_path).name,
